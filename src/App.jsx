...
function classifyConsult(text) {
  ...
  if (v.includes("疲れ") || v.includes("リラックス") || v.includes("癒") || v.includes("眠") || v.includes("気分") || v.includes("しんどい") || v.includes("ストレス"))
    return {
      category: "疲れ・気分・リラックス", group: "来店前",
  ...
  if (v.includes("初めて") || v.includes("はじめて") || v.includes("不安") || v.includes("怖い") || v.includes("美容室"))
    return {
      category: "美容室選びの不安", group: "来店前",
      summary: "美容室選びや来店前の不安に関する相談として記録しました。",
  ...
  if (v.includes("予約") || v.includes("メニュー") || v.includes("迷い") || v.includes("迷って") || v.includes("何がいい") || v.includes("どうすれば") || v.includes("どうしたら"))
    return {
      category: "メニュー・予約前の迷い", group: "来店前",
      summary: "メニュー選びや予約前の迷いに関する相談として記録しました。",
      guidance: "メニューの最終判断は実際の髪の状態を見てからになります。ご予約時の備考欄に気になることを書いていただくか、お急ぎの場合はお電話でご相談ください。",
      nextAction: "ヘアスタイルや施術内容の最終判断は、髪の状態を確認したうえでご案内します。ご予約時の備考欄にご記入いただくか、お急ぎの場合はお電話でご相談ください。",
    };
  ...
  return {
    ...
    guidance: "髪や美容についての相談として受け取りました。詳しくは来店時の髪の状態を確認したうえでご案内します。",
    nextAction: "ヘアスタイルや施術内容の最終判断は、髪の状態を確認したうえでご案内します。ご予約時の備考欄にご記入いただくか、お急ぎの場合はお電話でご相談ください。",
  };
}
